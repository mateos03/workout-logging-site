...
<div class="">
  <div class="py-5 border-b flex justify-between items-center">
    <div class="text-3xl">Exercises</div>
    <a href="/exercises/new" class="border rounded-lg p-1.5 bg-green-800">New Exercise</a>
  </div>
  <div class="border-b">
  ...
  {#each data.exercises.filter((exercise) => exercise.name.toLowerCase().includes(searchInput.toLowerCase())) as exercise}
    ...
    <div class="py-4 pt-3 border-b flex justify-start items-center w-full gap-x-3">
      <a href="/exercises/edit/{exercise.id}" class="w-1/2 text-lg border rounded-md p-1 py-1.5 bg-slate-800 text-center">Edit</a>
      <form class="w-1/2" method="POST">
        <button name="exercise_id" value={exercise.id} class="bg-red-800 p-1 py-1.5 w-full rounded-md border text-lg" onclick={(event) => confirmDelete(event, `Are you sure you want to delete ${exercise.name}`)}>Delete</button>
      </form>
    </div>
  {/each}
</div>