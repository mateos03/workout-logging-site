  ...
  <form class="" method="POST" use:enhance={() => {
    return async ({result, update}) => {
      if(result.type === "success"){
        chosenTags = [];
        availableTags = data.tags.map((x) => x.id);
        update();
      } else {
        update();
      }
    ...
    <label class="text-xl block pt-4 py-2" for="exercise_name_input">
      Name:
    </label>
    <input class="bg-slate-800 rounded-md w-full text-2xl" name="exercise_name" id="exercise_name_input"/>
    <label class="block pt-4 py-2 text-xl" for="exercise_description_input">
      Description:
    </label>
    <textarea class="bg-slate-800 text-xl w-full rounded-md" name="exercise_description" id="exercise_description_input"></textarea>
    ...
    {/if}
    <button class="border rounded-md p-2 text-2xl bg-green-800 mt-4 mb-2 w-full">Add Exercise</button>
    <p class="text-xl text-red-500">{form?.message ?? ""}</p>
    <a href="/exercises" class="block border rounded-md p-2 text-2xl bg-slate-800 my-2 w-full text-center">Go Back</a>
  </form>
</div>