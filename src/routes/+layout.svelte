<script lang="ts">
	...
	let menuLinks: {name: string, link: string}[] = [{name: 'Home', link: ''}, {name: 'My Workouts', link: 'workouts'}, {name: 'Calendar', link: 'calendar'}];
	...
	<div class="bg-gray-900 min-h-screen text-white">
		...
		<div class="py-4 px-4 bg-slate-950 flex items-center justify-between border-b border-solid border-white">
			...
			<a href="/profile" onclick={() => menuOpen = false}>
				<img src="/profile.png" alt="Login button" class="h-10 w-auto relative">
			</a>
		</div>

		{#if menuOpen}
			<div class="bg-slate-950 absolute w-full">
...
{:else}
	<div class="bg-gray-900 min-h-screen text-white">
		<div class="px-8 py-4 bg-slate-950 flex items-center justify-between">
			<p>Something</p>
			<img src="/login.png" alt="Login button" class="h-9 w-auto">
		</div>
